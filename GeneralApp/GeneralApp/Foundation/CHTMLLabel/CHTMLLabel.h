...
typedef NS_ENUM(NSUInteger, CHTMLLabelVerticalAlignment) {
    CHTMLLabelVerticalAlignmentCenter   = 0,
    CHTMLLabelVerticalAlignmentTop      = 1,
    CHTMLLabelVerticalAlignmentBottom   = 2,
};

@class CHTMLLabel;

@protocol CHTMLLabelDelegate <NSObject>
 ...
 */
- (void)CHTMLLabel:(CHTMLLabel *)label didSelectLinkWithURL:(NSURL*)URL;
 ...
 */
- (void)CHTMLLabel:(CHTMLLabel *)label didHoldLinkWithURL:(NSURL*)URL;

@end

/**
 `CHTMLLabel` is a lightweight, easy to use replacement for `UILabel` which allows you to fully customize the appearence of the text using HTML.
 
 # Differences Between `CHTMLLabel` and `UILabel`
 
 For the most part, `CHTMLLabel` behaves the same as `UILabel`. The following are notable exceptions, in which `CHTMLLabel` properties may act differently:
...
/**
 ...
 @discussion A `CHTMLLabelDelegate` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */

@property (nonatomic, assign) IBOutlet NSObject <CHTMLLabelDelegate> *delegate;
...
/**
 The length of time the user most hold a link to trigger the 'CHTMLLabel:didHoldLinkWithURL' delegate method. The value is 0.5 by default.
 */
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `CHTMLLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) CHTMLLabelVerticalAlignment verticalAlignment;
