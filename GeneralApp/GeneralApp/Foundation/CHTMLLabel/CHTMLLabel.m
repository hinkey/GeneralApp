...
#pragma mark - CHTMLLabel

@interface CHTMLLabel ()
...
- (NSAttributedString *)applyStylesToString:(NSString *)string
{
    ...
    for (HTMLComponent *component in styleComponents) {
                         ...
                         range:(NSRange)range
{
    ...
    BOOL forceCustomFont = (self.customItalicFontName == nil)?NO:YES;
                       ...
                       range:(NSRange)range
{
    ...
    BOOL forceCustomFont = (self.customBoldFontName == nil)?NO:YES;
                             ...
                             range:(NSRange)range
{
    ...
    BOOL forceCustomFont = (self.customBoldItalicFontName == nil)?NO:YES;
    ...
    if ([value isKindOfClass:[UIColor class]]) {
        ...
        if ([value rangeOfString:@"#"].location == 0) {
            ...
            if ([UIColor respondsToSelector:selector]) {
                ...
                CGColorRef ccolor = [color CGColor];
                CFAttributedStringSetAttribute(text, CFRangeFromNSRange(range), kCTForegroundColorAttributeName, ccolor);
            }
        }
    }
}
    ...
    while (!scanner.isAtEnd) {
        ...
        if ([text rangeOfString:@"</"].location == 0) {
            ...
            if (position != NSNotFound) {
                ...
                for (NSInteger i = components.count - 1; i >= 0; i--) {
                    HTMLComponent *component = components[i];
        ...
        else
        {
            ...
            if (htmlTag.length > 0) {
                ...
                HTMLComponent *component = [[HTMLComponent alloc] initWithString:nil htmlTag:htmlTag attributes:attributes];
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < textRect.size.height) {
        ...
        {
            case CHTMLLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case CHTMLLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case CHTMLLabelVerticalAlignmentTop:
            default:
                break;
        }
...
{
    CHTMLLabel *label = [[CHTMLLabel alloc] initWithFrame:CGRectMake(0.0, 0.0, size.width, size.height)];
    label.font = font;
           ...
           withEvent:(UIEvent *)event
{
    ...
    if (self.activeLink) {
        ...
        if ([self.delegate respondsToSelector:@selector(CHTMLLabel:didSelectLinkWithURL:)]) {
            [self.delegate CHTMLLabel:self didSelectLinkWithURL:result.URL];
            return;
        }
    } else {
...
- (void)handleDidHoldTouch:(NSTimer *)timer
{
    ...
    if ([self.delegate respondsToSelector:@selector(CHTMLLabel:didHoldLinkWithURL:)]) {
        NSTextCheckingResult *result = self.activeLink;
        self.activeLink = nil;
        
        [self.delegate CHTMLLabel:self didHoldLinkWithURL:result.URL];
    }
}
