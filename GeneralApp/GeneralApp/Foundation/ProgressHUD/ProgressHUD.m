...
- (void)showWithStatus:(NSString*)string maskType:(ProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show style:(UIStatusBarStyle)style;
...
+ (ProgressHUD*)sharedView
{
    ...
    dispatch_once(&once, ^ { sharedView = [[ProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
    return sharedView;
}
...
+ (void)show
{
    [[ProgressHUD sharedView] showWithStatus:nil maskType:ProgressHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithStyle:(UIStatusBarStyle)style
{
    [[ProgressHUD sharedView] showWithStatus:nil maskType:ProgressHUDMaskTypeNone networkIndicator:NO style:style];
}

+ (void)showWithStatus:(NSString *)status
{
    [[ProgressHUD sharedView] showWithStatus:status maskType:ProgressHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithStatus:(NSString *)status style:(UIStatusBarStyle)style
{
    [[ProgressHUD sharedView] showWithStatus:status maskType:ProgressHUDMaskTypeNone networkIndicator:NO style:style];
}

+ (void)showWithMaskType:(ProgressHUDMaskType)maskType
{
    [[ProgressHUD sharedView] showWithStatus:nil maskType:maskType networkIndicator:NO];
}

+ (void)showWithMaskType:(ProgressHUDMaskType)maskType style:(UIStatusBarStyle)style
{
    [[ProgressHUD sharedView] showWithStatus:nil maskType:maskType networkIndicator:NO style:style];
}

+ (void)showWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType
{
    [[ProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO];
}

+ (void)showWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType style:(UIStatusBarStyle)style
{
    [[ProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO style:style];
}

+ (void)showSuccessWithStatus:(NSString *)string
{
    [ProgressHUD showSuccessWithStatus:string duration:1];
}

+ (void)showSuccessWithStatus:(NSString *)string style:(UIStatusBarStyle)style
{
    [ProgressHUD showSuccessWithStatus:string duration:1 style:style];
}

+ (void)showSuccessWithStatus:(NSString *)string duration:(NSTimeInterval)duration
{
    [ProgressHUD show];
    [ProgressHUD dismissWithSuccess:string afterDelay:duration];
}

+ (void)showSuccessWithStatus:(NSString *)string duration:(NSTimeInterval)duration style:(UIStatusBarStyle)style
{
    [ProgressHUD showWithStyle:style];
    [ProgressHUD dismissWithSuccess:string afterDelay:duration];
}

+ (void)showErrorWithStatus:(NSString *)string
{
    [ProgressHUD showErrorWithStatus:string duration:1];
}

+ (void)showErrorWithStatus:(NSString *)string style:(UIStatusBarStyle)style
{
    [ProgressHUD showErrorWithStatus:string duration:1 style:style];
}

+ (void)showErrorWithStatus:(NSString *)string duration:(NSTimeInterval)duration
{
    [ProgressHUD show];
    [ProgressHUD dismissWithError:string afterDelay:duration];
}
...
- (void)showWithStatus:(NSString*)string maskType:(ProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show style:(UIStatusBarStyle)style
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        ProgressHUDViewController *viewController = (ProgressHUDViewController *)self.overlayWindow.rootViewController;
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (error) {
            self.imageView.image = [UIImage imageNamed:@"hud_error_white.png"];
        } else {
            self.imageView.image = [UIImage imageNamed:@"hud_success_white.png"];
        }
...
- (UIWindow *)overlayWindow
{
    if(!overlayWindow) {
        ...
        overlayWindow.rootViewController = [[ProgressHUDViewController alloc]init];
    }
